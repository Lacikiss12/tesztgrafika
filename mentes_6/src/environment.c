...
void draw_statue_group(const Scene* scene) {
    ...
    glPushMatrix();
        ...
        draw_model(&(scene->statue)); // Ha ez a kocka modellje
    glPopMatrix();
   ...
   // 2. FORGÓ EGYSÉG (A pillér tetején)
    ...
    // --- Korong (Kör objektum) ---
    glPushMatrix();
        glScalef(0.4f, 0.4f, 0.05f); // Kicsit szélesebb korong (0.4)
        draw_model(&(scene->statue)); 
    glPopMatrix();

    // --- Szobor (A korong tetején) ---
    glPushMatrix();
        // Mivel a korong elforgott, és ezen a blokkon belül vagyunk, 
        // a szobor is elforgatva kerül ide. Csak meg kell emelnünk a korong tetejére.
