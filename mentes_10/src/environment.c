...
void draw_statue_group(const Scene* scene, const Model* current_model, GLuint current_texture) {
    // 1. PILLÉR (Fix alapozás - mindenhol ugyanaz)
    glPushMatrix();
        glBindTexture(GL_TEXTURE_2D, scene->wall_texture); 
    ...
    // 2. FORGÓ EGYSÉG (A pillér tetején minden, ami mozog)
    glPushMatrix();
        ...
        // --- A SZOBOR (Paraméterezve) ---
        glPushMatrix();
            glTranslatef(0.0f, 0.0f, 0.01f); 
            glBindTexture(GL_TEXTURE_2D, current_texture); // Az átadott textúra

            glRotatef(90.0f, 1.0f, 0.0f, 0.0f); 

            glScalef(0.04f, 0.04f, 0.04f); 
            draw_model(current_model); // Az átadott modell
        glPopMatrix();
    glPopMatrix();

    // 3. VITRIN (Meghívjuk a korábban megírt üveget is)
    draw_vitrine(scene);
}
